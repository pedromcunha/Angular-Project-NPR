...
var app = angular.module('appControllerModule', []);
 ...
 //    	};
	...
	app.controller('headerController', ['$scope', '$sce', 'apiKeys', 'rottenTomatoesService', 'youtubeApiService', 'sharedVideos', function($scope, $sce, apiKeys, rottenTomatoesService, youtubeApiService, sharedVideos) {
		$scope.autocompleteSearch = function (input) {
			var url = "http://api.rottentomatoes.com/api/public/v1.0/movies.json?apikey=" + apiKeys.rottenTomatoesAPI + "&q="+input.split(' ').join('+')+"&page_limit=5&callback=JSON_CALLBACK",
			 	autoSuggest = [],
			 	ratings = [],
	...
	app.controller('videoListingController', ['$scope', 'sharedVideos', function($scope, sharedVideos) {
